...
> 即给每个 Kubernetes 组件做一个容器镜像，然后在每台宿主机上用 docker run 指令启动这些组件容器
...
4. kubelet 在 Kubernetes 项目中的地位非常高，在设计上它就是一个`完全独立的组件`，而其他 Master 组件，则更像是辅助性的系统容器
...
3. 生成 Kubernetes 对外提供服务所需的`各种证书`和`对应的目录`
